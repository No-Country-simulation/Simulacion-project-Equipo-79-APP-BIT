 ...
 * <p>
 * Proporciona endpoints para crear nuevas ofertas de trabajo, listar todas las
 * ofertas
...
public class JobController {
         ...
         * </p>
         *
         * @param JobRequest objeto {@link JobRequest} con los datos de la oferta a
         *                   crear.
         *                   Debe contener:
         *                   <ul>
         *                   <li><b>title</b>: Título del puesto (obligatorio)</li>
         *                   <li><b>description</b>: Descripción detallada del puesto
         *                   (opcional)</li>
         *                   <li><b>region</b>: Región geográfica donde se ubica el
         *                   puesto (opcional)</li>
         *                   <li><b>requiredSkills</b>: Lista de habilidades técnicas
         *                   requeridas (opcional)</li>
         *                   <li><b>experienceLevel</b>: Nivel de experiencia requerido
         *                   (obligatorio)</li>
         *                   <li><b>companyId</b>: ID de la empresa que publica la
         *                   oferta (obligatorio)</li>
         *                   </ul>
         * @return la oferta de trabajo recién creada con código HTTP 201 (Created).
         */
        @PostMapping
        @Operation(summary = "Crear una nueva oferta de trabajo", description = "Crea una nueva oferta de trabajo (job) en el sistema. "
                        +
                        "La oferta queda asociada a una empresa existente mediante el companyId. " +
                        "Devuelve la oferta con el ID generado automáticamente y la fecha de publicación.", requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Datos de la oferta de trabajo a crear", required = true, content = @Content(mediaType = "application/json", schema = @Schema(implementation = JobRequest.class))))
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "201", description = "Oferta de trabajo creada exitosamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = JobResponse.class))),
                        ...
                        @ApiResponse(responseCode = "404", description = "Empresa no encontrada: el companyId proporcionado no existe", content = @Content(mediaType = "application/json")),
                        @ApiResponse(responseCode = "500", description = "Error interno del servidor", content = @Content(mediaType = "application/json"))
        })
        public ResponseEntity<JobResponse> createJob(
                        @Parameter(description = "Datos de la oferta de trabajo a crear", required = true) @Valid @RequestBody JobRequest job) {
         ...
         */
        @GetMapping
        @Operation(summary = "Listar todas las ofertas de trabajo", description = "Recupera un listado completo de todas las ofertas de trabajo (jobs) "
                        +
                        "registradas en el sistema. No aplica paginación ni filtros.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Lista de ofertas de trabajo obtenida exitosamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = JobResponse.class))),
                        @ApiResponse(responseCode = "500", description = "Error interno del servidor", content = @Content(mediaType = "application/json"))
        })
         ...
         * @return la oferta de trabajo encontrada con código HTTP 200 (OK), o 404 si no
         *         existe.
         */
        @GetMapping("/{id}")
        @Operation(summary = "Obtener oferta de trabajo por ID", description = "Recupera los datos de una oferta de trabajo específica utilizando su identificador único. "
                        +
                        "Devuelve un código 404 si la oferta no existe en el sistema.", parameters = {
                                        @Parameter(name = "id", description = "Identificador único de la oferta de trabajo", required = true, example = "1")
                        })
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Oferta de trabajo encontrada exitosamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = JobResponse.class))),
                        @ApiResponse(responseCode = "404", description = "Oferta de trabajo no encontrada con el ID proporcionado", content = @Content(mediaType = "application/json")),
                        @ApiResponse(responseCode = "500", description = "Error interno del servidor", content = @Content(mediaType = "application/json"))
        })
        public ResponseEntity<JobResponse> getJobById(
                        @Parameter(description = "ID único de la oferta de trabajo", required = true, example = "1") @PathVariable Long id) {
        ...
        /**
         * Busca candidatos compatibles con una oferta de trabajo usando el motor de
         * matching con IA.
         *
         * @param request datos de la vacante (título, habilidades, nivel de experiencia
         *                y región).
         *                Todos los campos son obligatorios y son validados con
         *                {@code @Valid}.
         * @return lista de resultados de compatibilidad con código HTTP 200 (OK).
         */
        @PostMapping("/matches")
        @Operation(summary = "Buscar candidatos compatibles con una oferta", description = "Ejecuta el motor de matching con inteligencia artificial para encontrar "
                        +
                        "candidatos compatibles con la vacante especificada. " +
                        "Los datos de la vacante se validan antes de ser procesados.", requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Datos de la vacante para el matching", required = true, content = @Content(mediaType = "application/json", schema = @Schema(implementation = JobMatchRequest.class), examples = {
                                        @ExampleObject(name = "Ejemplo de Vacante Backend", description = "Ejemplo de JSON válido para buscar un Desarrollador Backend MID en Florianopolis", value = "{\n"
                                                        +
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Lista de candidatos compatibles obtenida exitosamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = MatchResultResponse.class))),
                        @ApiResponse(responseCode = "400", description = "Solicitud inválida: campos obligatorios faltantes o vacíos", content = @Content(mediaType = "application/json")),
                        @ApiResponse(responseCode = "500", description = "Error interno del servidor", content = @Content(mediaType = "application/json"))
        })
